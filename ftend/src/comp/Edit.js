...
const AddPost = () => {
    const { id } = useParams(); // Get the job ID from the URL (if editing)
    ...
    // Fetch job details when id changes (for editing)
    useEffect(() => {
        if (id) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (
            ...
            try {
                ...
                if (id) {
                    // Update existing job
                    res = await axios.put(`http://localhost:5000/edit/${id}`, formdata);
                } else {
                    // Create new job
                    res = await axios.post("http://localhost:5000/addjob", formdata);
                }
                alert(res.data.msg);
        ...
        <div className="main">
            <div className="addpost-container" >
                <div className="addpost-card">
                    <h2 className="addpost-title">{id ? "Edit Job Details" : "Post the Job Details"}</h2>
                    <form className="addpost-form" onSubmit={handleSubmit}>
                        ...
                        <button type="submit" className="addpost-button" style={{ margin: "auto" }}>
                            Update
                        </button>
                    </form>
                </div>
                ...
                {showPreview && (
                    <div className="preview-container">
                        ...
                        <div className="preview-content">
                            ...
                            {formdata.joboverview && <p style={{ textAlign: "justify" }}><strong>Job Overview:</strong><br /> {formdata.joboverview}</p>}
